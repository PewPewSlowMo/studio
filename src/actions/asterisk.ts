...
async function sendToAri(
  ...
  try {
    ...
    if (response.status === 204) { // No Content, success for DELETE and some POSTs
        return { success: true };
    }
    ...
    if (e instanceof Error) {
        if (e.message.includes('fetch failed')) {
            return { success: false, error: 'Network error: Could not reach the host.' };
        }
        return { success: false, error: e.message };
    }
    return { success: false, error: 'An unknown error occurred.' };
...
export async function answerCallAri(
  connection: AsteriskConnection,
  channelId: string
): Promise<{ success: boolean; error?: string }> {
    return sendToAri(connection, `channels/${channelId}/answer`, 'POST');
}

export async function hangupCallAri(
    connection: AsteriskConnection,
    channelId: string
): Promise<{ success: boolean; error?: string }> {
    return sendToAri(connection, `channels/${channelId}`, 'DELETE');
}
